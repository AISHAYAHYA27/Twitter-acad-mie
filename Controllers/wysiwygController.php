...
// var_dump($_FILES);
// echo "</pre>";
// $jeFollow = mentionFollowings($_COOKIE['Twitter_connected']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    for ($t = 1; $t <= count($_POST); $t++) 
    {
        ...
        if (isset($_POST["tweet{$t}"]['content']) && !empty($_POST["tweet{$t}"]['content'])) 
        {
            $cols .= ", content";
            $values .= ", :content";
            $execute['content'] = inputSanitize(nl2br($_POST["tweet{$t}"]['content']));
            ...
            if (isset($_FILES["tweet{$t}_medias"]) && !empty($_FILES["tweet{$t}_medias"]['name'])) 
            {
                ...
                if ($n <= count($_FILES["tweet{$t}_medias"]['name']) && count($_FILES["tweet{$t}_medias"]['name']) <= 4) 
                {
                    ...
                    for ($i = 0; $i < count($_FILES["tweet{$t}_medias"]['name']); $i++)
                    {
                        ...
                        {
                            $wswgMsg = "Erreur lors du téléchargement du fichier {$_FILES["tweet{$t}_medias"]['name'][$i]}." . PHP_EOL;
                            continue;
                        }
                        ...
                        {
                            $wswgMsg = "{$_FILES["tweet{$t}_medias"]['name'][$i]} est trop lourd" . PHP_EOL;
                        } 
                        else 
                        {
                            ...
                            {
                                $wswgMsg = "Téléchargement du {$name} échoué" . PHP_EOL;
                            }
                ...
                {
                    $wswgMsg = "Vous ne pouvez ajouter que 4 médias par tweet... le téléchargement n'a pas pu aboutir." . PHP_EOL;
                }
            ...
            {
                $wswgMsg = "Aucun média n'a été téléchargé !" . PHP_EOL;
            } 
        ...
        {
            $wswgMsg = "Votre tweet ne contient pas de message !" . PHP_EOL;
        }
    }
}
